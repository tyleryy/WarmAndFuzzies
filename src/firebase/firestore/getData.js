...
export async function getAllMembers(collect) {
  ...
  try {
    ...
    result = result.docs.map((doc) => {
      return { checklist: doc.data(), user: doc.id };
    });

    for (let doc of result) {
      // ! async calls don't work in a forEach loop, so opted for traditional
      ...
      let payload = {
        data: doc.checklist.data.filter((elem) => {
          return elem.checked;
        }),
        user_data: user_data,
      };
  ...
  output?.sort((elem, elem2) => {
    return elem2.data.length - elem.data.length;
  });
  if (output.length >= 2) [output[0], output[1]] = [output[1], output[0]];
  return { output, error };
}
